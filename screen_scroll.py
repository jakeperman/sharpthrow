...
class ScrollManager:
    def __init__(self, window: arcade.Window):
        ...
        self.views = {'right': self.window.map.spawn_x + (self.width/2), 'left': self.window.map.spawn_x - (self.width/2), 'top': self.height, 'bottom': 0}
    ...
    def check_views(self):
        ...
        limits = self.limits
        right = False
        if self.views['right'] > limits['right']:
